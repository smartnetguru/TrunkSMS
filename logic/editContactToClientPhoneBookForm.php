...
$phoneNo = $_SESSION['phoneNo']; //session variable
	...
	if(isset($_POST['submit'], $_POST['phoneNo'], $_POST['contactName'], $_POST['category'], $_POST['id'], $phoneNo )){
	$name = ucwords(strtolower(addslashes(trim($_POST['contactName']))));
	$category = addslashes(trim($_POST['category']));
	$number = addslashes(trim($_POST['phoneNo']));
	$id = (int) $_POST['id'];
		if(strlen($category) == 0 || strlen($name) > 255 || strlen($category) > 255 || strlen($name) == 0 || strlen($number) == 0 || strlen($number) > 255){
		echo "<div class=\"ui-widget\">";
echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em;  margin-top: 20px; \">";
				echo "<p><span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"; 
				echo "<strong>Alert:</strong> Please check the entry and fill them properly. some fields are empty.</p>";
...
/*
		echo "<div id = \"failure\">";
	echo "Please check the entry and fill them properly. some fields are empty";
	...
	*/
		...
		$num = @mysql_num_rows($result);
			...
			$sql = "UPDATE TRUNKphoneBook SET `number` = '$number', `name` = '$name', `category` = '$catID' WHERE `id` = $id";
			//$sql = "INSERT INTO phoneBook (`id`, `phoneNo`, `number`, `name`, `category`) VALUES (NULL, '$phoneNo', '$number', '$name', '$category');";
			$result = @mysql_query($sql);
				if($result){
				...
				/*
				echo "<div id = \"success\">";
				echo "Entry was Successfuly edited";
...
echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em; margin-top: 20px; \">";
				echo "<p><span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"; 
				echo "<strong>Alert:</strong> The provided Entry Exists.</p>";
				echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getContactForm();return false;\">back</a> | <a style = \"text-decoration: none;\" href = \"./\" onClick = \"getSMSForm(); return false;\">Send SMS</a></div>";
				echo "</div></div>";
				
				/*
			echo "<div id = \"failure\">";
			echo "The provided Entry Exists.";
			echo "</div> <!-- end failure -->";
			echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getContactForm();return false;\">back</a> | <a style = \"text-decoration: none;\" href = \"./\" onClick = \"getSMSForm(); return false;\">Send SMS</a></div>";	*/
