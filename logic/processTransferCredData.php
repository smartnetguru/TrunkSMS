...
$phoneNo = $_SESSION['phoneNo']; //session variable
	...
	if(isset($_POST['submit'], $_POST['acctno'], $_POST['units'])){
	$acctno = addslashes(trim($_POST['acctno']));
	$units = (int) addslashes(trim($_POST['units']));
	
	$sql = "SELECT * FROM TRUNKregistration WHERE AccountNo = '$acctno' AND phoneNo = '$phoneNo' "; //this code has flaws
	$num = 0;
...
echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em;  margin-top: 20px; \">";
				...
				echo "<strong>Alert:</strong> Ooops! you cannot send units to your self.</p>";
				echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";	
echo "</div></div>";
	/*
	echo "<div id = \"failure\">";
	echo "Ooops! you cannot send units to your self.";
	...
	*/
	...
	echo "<div class=\"ui-widget\">";
				echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em;  margin-top: 20px; \">";
				echo "<p><span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"; 
				echo "<strong>Alert:</strong> Ooops! Invalid Units.</p>";
					echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";
				echo "</div></div>";
				/*
	echo "<div id = \"failure\">";
	echo "Ooops! Invalid Units.";
	echo "</div> <!-- end failure -->";
	echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";*/
	exit(1);
	}
	
		if(strlen($acctno) == 0 || strlen($acctno) > 255 || strlen($units) > 255 || strlen($units) == 0 || $units == 0 ){
		echo "<div class=\"ui-widget\">";
				echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em;  margin-top: 20px; \">";
				echo "<p><span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"; 
	...
	echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";*/
		...
		$SenderUnits = $result['SMSunits'];
		
		if($units > $SenderUnits){
		showerror();
		exit();
		}
		
		$sql = "SELECT * FROM TRUNKregistration WHERE AccountNo = '$acctno' ";
		$result = @mysql_query($sql);
		$num = 0;
		$num = @mysql_num_rows($result);
		
		
			if($num == 1){ //the acct no to be credited exists
			$result = @mysql_fetch_array($result);
			$reciepientOldUnits = $result['SMSunits'];
			$recipientPhoneNo = $result['phoneNo'];
			
			$newSenderUnits = $SenderUnits - $units;
			$newreciepientUnits = $units + $reciepientOldUnits;
			
			$sql = "UPDATE TRUNKregistration SET `SMSunits` = '$newSenderUnits' WHERE `TRUNKregistration`.`phoneNo` = '$phoneNo' ";
			$result = @mysql_query($sql);
			
			$sql = "UPDATE TRUNKregistration SET `SMSunits` = '$newreciepientUnits' WHERE `TRUNKregistration`.`phoneNo` = '$recipientPhoneNo' ";
			$result2 = @mysql_query($sql);
			
				if($result && $result2){ //both parties went successfuly
				
				$_SESSION['balance'] = $newSenderUnits;
				echo "<div class=\"ui-widget\">";
				...
				echo "Transaction Completed Successfuly</p>";
...
echo "<div class=\"ui-state-error ui-corner-all\" style=\"padding: 0 .7em; margin-top: 20px; \">";
				echo "<p><span class=\"ui-icon ui-icon-alert\" style=\"float: left; margin-right: .3em;\"></span>"; 
				echo "<strong>Alert:</strong> The provided Account Number does not Exists.</p>";
				echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";
				echo "</div></div>";
				/*
			echo "<div id = \"failure\">";
			echo "The provided Account Number does not Exists.";
			echo "</div> <!-- end failure -->";
			echo "<div  class = \"back\"><a style = \"text-decoration: none;\" href = \"./\" onClick = \"getTransferSMSForm(); return false\">back</a></div>";
			*/
