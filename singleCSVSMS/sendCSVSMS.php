...
if(strlen($_SESSION['phoneNo']) > 0){
	if(!empty($_REQUEST['csv']) && !empty($_REQUEST['smsmessage']) && !empty($_REQUEST['smsname'])){
	$csv = $_REQUEST['csv'];
	...
	$smsname = $_REQUEST['smsname'];
				...
				foreach($csv_array as $number_from_csv)
				{
					if(strlen($number_from_csv) >= 11)
					sheduleSMS($_SESSION['phoneNo'], $mesg, $smsname, $number_from_csv,$times);
				}
				
				$_SESSION['message'] .= "<p>Dear User, Your SMS Will be sent to " . $csv . " ". $date . " at " . $time . "<br/></p>";
	...
	foreach($csv_array as $number_from_csv)
	{
		if($units > 1 || $units == 1){
					...
					if(strlen($number_from_csv) >= 11)
					sendSMS($number_from_csv,$smsname,$mesg);
